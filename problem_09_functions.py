...
class PizzaProcessor:
    ...
    def pizza_pick(self, name, crust, num_toppings):
        if isinstance(name, str) and isinstance(crust, str) and isinstance(num_toppings, int) and num_toppings in range(0,11):
            cap_name = name.capitalize()
            lower_crust = crust.lower()
            return f"{cap_name} ordered a {lower_crust} crust pizza with {num_toppings} toppings!"
        else:
            return self._error
    def pizza_production(self, employee_name=None, time_ended=None, time_started=None):
    ...
    def pizza_prepper(self, *pizzas):
        ...
        for i,p in enumerate(pizzas):
            if isinstance(p, int) and p in range(0,11):
                if i % 2 == 0:
                    new_dict[str(i)] = {"crust": "thin", "toppings": p}
                else:
                    new_dict[str(i)] = {"crust": "stuffed", "toppings": p}
                self._pizzas_made += 1
            else:
                return self._error
        return new_dict
    def pizza_printer(self, **pizza_dict):
        base_str =  "Printing all the pizzas!"
        for key,val in pizza_dict.items():
